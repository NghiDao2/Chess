...
const char* MOVE_TYPESTR_2[16] = {
	"", "", "O-O", "O-O-O", "n", "b", "r", "q", "", "", "", "",
	"n", "b", "r", "q"
};

std::string Move::to_string() {
	if (this->flags() == OO || this->flags() == OOO) {
		return std::string(MOVE_TYPESTR_2[this->flags()]);
	}
	return std::string(SQSTR[this->from()]) + std::string(SQSTR[this->to()]) + std::string(MOVE_TYPESTR_2[this->flags()]);
}
