...
public:
	...
	inline Piece piece_promotion() const {
		if ((move >> 14) == 1) {
			return Piece((move >> 12)&&0b11 + 1);
		}
		return NO_PIECE;
	}

	inline bool is_promotion() const {
		return (((this->flags()) & 0b0100) == 0b0100);
	}

	inline bool is_enpassant() const {
		return (this->flags()) == EN_PASSANT;
	}
	...
	uint16_t to_int16() {return this->move;}
};
